...
public class Controller {
    ...
    private boolean isValid() {
        ...
        YearLevel year = cbYear.getValue();

        if (name.isEmpty() || course.isEmpty() || year == null) {
            showAlert("All fields are required. Please fill in Name, Course, and Year Level.");
            return false;
        }
        return true;
    }
    ...
    private boolean isRowSelected() {
        if (selectedId == -1) {
            showAlert("Please select a student from the table first.");
            return false;
        }
        return true;
    }
